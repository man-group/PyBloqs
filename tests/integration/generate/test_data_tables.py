...
FLAT_DATA = pd.DataFrame(TABLE_DATA,
                         pd.date_range("2000-01-01", freq="B", periods=7),
                         columns=['a', 'b', 'c', 'd', 'e'])
FLAT_DATA.index.name = 'Index'


@assert_report_generated
def test_df_to_data_tables_jinja_table_default():
    return DataTablesHTMLJinjaTableBlock(FLAT_DATA, use_default_formatters=False)


@assert_report_generated
def test_df_to_data_tables_jinja_table_add_extra_formatters():
    ...
    return DataTablesHTMLJinjaTableBlock(FLAT_DATA, formatters=formatters, use_default_formatters=False)


@assert_report_generated
def test_df_to_data_tables_jinja_table_extra_formatters_only():
    return DataTablesHTMLJinjaTableBlock(FLAT_DATA, formatters=[blformat.FmtHeatmap(threshold=0., axis=0)],
                                         use_default_formatters=False)


@assert_report_generated
def test_multi_index_df_to_data_tables_jinja_table():
    ...
    return DataTablesHTMLJinjaTableBlock(data, formatters=formatters, use_default_formatters=False)
